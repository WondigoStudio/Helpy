...
async def update_user(user_id, chat_id):
    c.execute("SELECT * FROM users WHERE user_id=? AND chat_id=?",
              (user_id, chat_id))
    if not c.fetchone():
        c.execute("INSERT INTO users (user_id, chat_id) VALUES (?, ?)",
                  (user_id, chat_id))
        conn.commit()
...
async def unmute_later(context, chat_id, user_id, until):
    ...
    c.execute("UPDATE users SET mute_until=NULL WHERE user_id=? AND chat_id=?",
              (user_id, chat_id))
    conn.commit()
...
async def warn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message:
        await update.message.reply_text(
            "Используйте /warn в ответ на сообщение пользователя.")
        return
    ...
    c.execute("SELECT warns FROM users WHERE user_id=? AND chat_id=?",
              (user.id, chat_id))
    warns = c.fetchone()[0]
    await update.message.reply_text(
        f"Предупреждение {user.mention_html()} | Всего: {warns}",
        parse_mode='HTML')

    if warns >= 2:
        until = datetime.utcnow() + timedelta(hours=24)
        permissions = ChatPermissions(can_send_messages=False)
        await context.bot.restrict_chat_member(chat_id,
                                               user.id,
                                               permissions,
                                               until_date=until)
        c.execute(
            "UPDATE users SET warns=0, mutes=mutes+1, mute_until=? WHERE user_id=? AND chat_id=?",
            (until.isoformat(), user.id, chat_id))
        conn.commit()
        await update.message.reply_text(
            f"{user.mention_html()} замучен на 24 часа за 2 предупреждения.",
            parse_mode='HTML')
...
async def mut(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message or not context.args:
        await update.message.reply_text(
            "Используйте /mut в ответ на сообщение: /mut 5m")
        return
    user = update.message.reply_to_message.from_user
    chat_id = update.effective_chat.id
    ...
    if seconds is None:
        await update.message.reply_text(
            "Неверный формат времени. Примеры: 5m, 2h, 1d")
        return
    ...
    await update_user(user.id, chat_id)
    c.execute(
        "UPDATE users SET mutes = mutes + 1, mute_until=? WHERE user_id=? AND chat_id=?",
        (until.isoformat(), user.id, chat_id))
    conn.commit()
    await update.message.reply_text(
        f"{user.mention_html()} замучен на {duration}.", parse_mode='HTML')
    asyncio.create_task(unmute_later(context, chat_id, user.id, until))
...
async def unmut(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message:
        await update.message.reply_text(
            "Используйте /unmut в ответ на сообщение пользователя.")
        return
    ...
    c.execute("UPDATE users SET mute_until=NULL WHERE user_id=? AND chat_id=?",
              (user.id, chat_id))
    conn.commit()
    await update.message.reply_text(f"{user.mention_html()} размучен.",
                                    parse_mode='HTML')
...
async def unwarn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message:
        await update.message.reply_text(
            "Используйте /unwarn в ответ на сообщение пользователя.")
        return
    ...
    if warns and warns[0] > 0:
        ...
        conn.commit()
        await update.message.reply_text(
            f"Предупреждение снято. Осталось: {warns[0]-1}")
    else:
        ...
        # Попробуем получить по @username
            ...
            try:
                ...
                if not isinstance(user, User):
                    await update.message.reply_text(
                        "Пользователь не найден или это не личный аккаунт.")
                    return
            except:
                await update.message.reply_text(
            ...
            await update.message.reply_text(
                "Неверный формат. Укажите ID или @username.")
            return
    else:
        await update.message.reply_text(
            "Используйте /rep в ответ на сообщение или укажите @username/ID.")
        return

    # Обновление и получение данных
    await update_user(user.id, chat_id)
    c.execute(
        "SELECT warns, mutes, mute_until FROM users WHERE user_id=? AND chat_id=?",
        (user.id, chat_id))
    result = c.fetchone()
    if not result:
...
async def ban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message:
        await update.message.reply_text(
            "⚠️ Эта команда должна быть ответом на сообщение пользователя.")
        return
    ...
    try:
        ...
        # Удаляем сообщение, вызвавшее команду
        await update.message.delete()

        # Уведомление
        await update.message.reply_text(
            f"🚫 Пользователь {user_to_ban.mention_html()} был забанен.",
            parse_mode="HTML")

        # Чат-экшен (анимация "удаление")
        ...
        # Удаляем все сообщения от забаненного
        for msg in messages:
            if msg.from_user and msg.from_user.id == user_to_ban.id:
                try:
                    await context.bot.delete_message(chat_id, msg.message_id)
                except:
                    pass  # если сообщение старое или не удаётся — игнорируем

    except Exception as e:
        await update.message.reply_text(
            f"❌ Не удалось забанить пользователя. Причина: {e}")
...
async def main():
    ...
    app = ApplicationBuilder().token(
        "8093659364:AAGsA9q45FSOomZFM-eeB4RrrL5TW1ZZQvg").build()
